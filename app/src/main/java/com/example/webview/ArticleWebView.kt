        ...
        event?.let {
            ...
            when (event.action) {
                ...
                MotionEvent.ACTION_MOVE -> {
                    slideDir =
                        kotlin.math.abs(currentX - startX) <= kotlin.math.abs(currentY - startY)
                }
        ...
        e?.let {
            for (child in e.children()) {
                if (child.tagName() == "code") {
                    ...
                    child.attributes().put(
                        "class",
                        "brush: ${getLanguageType(child.attr("class"))}; toolbar:false; quick-code:false;"
                    )
                }
                verseChild(child)
            }
